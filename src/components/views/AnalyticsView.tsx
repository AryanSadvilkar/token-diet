...
export const AnalyticsView: React.FC = () => {
  ...
  // Throughput chart points (k tokens/sec)
  const throughputPoints = [120, 145, 138, 180, 210, 195, 240, 260, 290, 310, 280, 340];
  const maxVal = Math.max(...throughputPoints);

  const algorithmPerformance = [
    { name: 'Cross-Encoder', reduction: '68%', latency: '14.2ms', accuracy: '99.4%', color: '#6EE7B7' },
    { name: 'BM25 Fast Score', reduction: '54%', latency: '4.8ms', accuracy: '96.2%', color: '#80f9c8' },
    { name: 'LLM-Semantic', reduction: '76%', latency: '38.5ms', accuracy: '99.8%', color: '#818CF8' },
    { name: 'HyDE Hybrid', reduction: '62%', latency: '18.1ms', accuracy: '98.7%', color: '#6EE7B7' },
  ];
  ...
  const endpointStats = [
    ...
    { endpoint: '/api/v1/sec-filings', calls: '310k', saved: '72%', latency: '16ms', status: 'Optimal' },
  ];
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <PageHeader
        ...
        title="Enterprise Telemetry & Token Analytics"
        description="Monitor real-time context compression ratios, token throughput velocity, and bill reduction across all active RAG routes."
      />

      {/* Browser Chrome Framed Dashboard Mockup */}
      <div className="border-technical bg-[#14141C]/90 backdrop-blur-xl rounded-2xl overflow-hidden shadow-2xl border-t border-white/10">
        ...
        <div className="bg-[#1C1C26] px-5 py-3.5 border-b border-[#2A2A38] flex items-center justify-between">
          ...
          <div className="flex items-center gap-2 font-mono-code text-xs">
            ...
            <span className="text-[#6EE7B7] text-[11px] font-bold">LIVE TELEMETRY ACTIVE</span>
          </div>
        </div>

        {/* Mockup Dashboard Body */}
        <div className="p-6 sm:p-8 space-y-6">
          {/* Top 4 Quick Telemetry Stat Cards */}
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 font-mono-code">
            <div className="bg-[#0A0A0F] border border-[#2A2A38] p-4 rounded-xl">
              <div className="text-[11px] text-[#888899] uppercase mb-1">Tokens Compressed</div>
            ...
            <div className="bg-[#0A0A0F] border border-[#2A2A38] p-4 rounded-xl">
              <div className="text-[11px] text-[#888899] uppercase mb-1">Avg Proxy Overhead</div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7 bg-[#0A0A0F] border border-[#2A2A38] p-5 rounded-xl space-y-4">
              ...
              <div className="h-48 w-full pt-2">
                <svg viewBox="0 0 500 160" className="w-full h-full overflow-visible" preserveAspectRatio="none">
                  <defs>
                    <linearGradient id="analyticsGradFull" x1="0%" y1="0%" x2="0%" y2="100%">
                  ...
                  {(() => {
                    ...
                    return (
                      <>
                        <path d={areaD} fill="url(#analyticsGradFull)" />
            ...
            <div className="lg:col-span-5 bg-[#0A0A0F] border border-[#2A2A38] p-5 rounded-xl space-y-3 font-mono-code text-xs">
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-[10px] text-[#6EE7B7]">5 Routes Live</span>
              </div>
                ...
                {endpointStats.map((item) => (
                  ...
                  >
                    <div>
                      ...
                      <div className="text-[10px] text-[#888899]">{item.calls} reqs • {item.latency}</div>
                    </div>
          ...
          {/* Secondary Algorithm Performance Benchmarks Bar Row */}
          <div className="bg-[#0A0A0F] border border-[#2A2A38] p-5 rounded-xl space-y-4 font-mono-code text-xs">
            <div className="flex justify-between items-center">
              <span className="text-[#e4e1ed] font-bold uppercase">Algorithm Pruning Benchmark Comparison</span>
              <span className="text-[#888899] text-[11px]">Sub-20ms P95 Target</span>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {algorithmPerformance.map((algo) => (
                <div key={algo.name} className="bg-[#14141C] p-3 rounded-lg border border-[#2A2A38] space-y-1.5">
                  <div className="text-[#bccac1] font-bold text-[11px]">{algo.name}</div>
                  <div className="flex justify-between items-baseline">
                    <span className="text-[#6EE7B7] font-bold text-base">{algo.reduction} cut</span>
                    <span className="text-[#888899] text-[10px]">{algo.latency}</span>
                  </div>
                  <div className="text-[10px] text-[#80f9c8]">Semantic Accuracy: {algo.accuracy}</div>
                </div>
              ))}
            </div>
          </div>
